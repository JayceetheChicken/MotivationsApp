...
function extractPartialStreams(input) {
    ...
    while (offset < input.length) {
        ...
        if (!jxlpBox)
            break;
        if (jxlpBox.size < 12)
            throw new TypeError('Invalid JXL partial codestream box');
        const nextOffset = jxlpBox.offset + jxlpBox.size;
        if (!Number.isSafeInteger(nextOffset) || nextOffset <= offset || nextOffset > input.length)
            throw new TypeError('Invalid JXL partial codestream offset');
